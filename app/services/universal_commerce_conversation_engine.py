...
class UniversalCommerceConversationEngine:
        ...
        if deals is not None:
            ...
            if seller_pending:
                ...
                if self.is_question(clean) and not self.looks_like_factual_answer(clean):
                    return ConversationDecision("UNRESOLVED_QUESTION", 0.75, "question conflicts with pending seller-detail state")
                return self._decision_from_deal("SELLER_DETAILS", 0.97, "seller is explicitly waiting to provide deal facts", seller_pending)

            buyer_summary = self._safe_call(deals, "pending_for_buyer_summary", sender)
            if buyer_summary:
                if self.is_question(clean):
                    return self._decision_from_deal("AUTO_BUYER_DOUBT", 0.95, "buyer has an active deal summary and asked a natural question", buyer_summary)
                if self.is_negotiation(clean):
                    return self._decision_from_deal("BUYER_DOUBT_OR_CHANGE", 0.93, "buyer requested a deal change/negotiation", buyer_summary)

        if self.is_question(clean):
            return ConversationDecision("GENERIC_COMMERCE_QUESTION", 0.82, "natural-language commerce question")
        if self.is_negotiation(clean):
            return ConversationDecision("GENERIC_NEGOTIATION", 0.80, "natural-language negotiation/change")
        if self.looks_like_factual_answer(clean):
            return ConversationDecision("GENERIC_FACTS", 0.72, "message contains factual commerce attributes")
        return ConversationDecision("GENERIC_TEXT", 0.50, "no strong commerce-state signal")
    ...
    @classmethod
    def _decision_from_deal(cls, intent: str, confidence: float, reason: str, deal: dict) -> ConversationDecision:
        return ConversationDecision(
            intent,
            confidence,
            reason,
