...
class AppContainer:
    def __init__(self) -> None:
        ...
        self.user_repository = UserRepository(self.database); self.inbound_message_repository = InboundMessageRepository(self.database)
        self.delivery_log_repository = DeliveryLogRepository(self.database); self.session_repository = SessionRepository(self.database)
        self.job_lifecycle_repository = JobLifecycleRepository(self.database); self.appointment_repository = AppointmentRepository(self.database)
        self.demand_repository = DemandRepository(self.database); self.marketplace_repository = MarketplaceRepository(self.database)
        ...
        self.catering_menu_pending_repository = CateringMenuPendingRepository(self.settings.database_path)
        ...
        self.intent_router_service = IntentRouterService(api_key=self.settings.gemini_api_key, model=self.settings.gemini_voice_model)
        self.appointment_service = AppointmentService(repository=self.appointment_repository, session_registry=self.session_registry)
        self.base_conversation_service = MarketplaceConversationService(user_repository=self.user_repository, session_registry=self.session_registry,
            intent_router=self.intent_router_service, marketplace_repository=self.marketplace_repository, appointment_service=self.appointment_service,
            demand_capture_service=self.demand_capture_service)
        self.whatsapp_service = WhatsAppService(access_token=self.settings.whatsapp_access_token, phone_number_id=self.settings.whatsapp_phone_number_id,
            api_version=self.settings.whatsapp_api_version)
        self.universal_notification_service = UniversalNotificationService(notification_repository=self.universal_notification_repository,
            whatsapp_service=self.whatsapp_service, contact_resolver=self._resolve_universal_contact)
        self.universal_response_command_service = UniversalResponseCommandService(demand_repository=self.universal_demand_repository,
            notification_service=self.universal_notification_service, notification_repository=self.universal_notification_repository)
        self.universal_request_extractor = UniversalRequestExtractor(api_key=self.settings.gemini_api_key, model=self.settings.gemini_voice_model)
        self.universal_matcher = UniversalMatcher(self.universal_demand_repository)
        self.universal_targeting_service = UniversalTargetingService(profile_source=self._universal_profiles,
            subject_similarity=UniversalMatcher._subject_similarity, distance_km=UniversalMatcher._distance_km)
        self.universal_live_capture_service = UniversalLiveCaptureService(extractor=self.universal_request_extractor,
            demand_repository=self.universal_demand_repository, matcher=self.universal_matcher, targeting_service=self.universal_targeting_service,
            notification_service=self.universal_notification_service, notification_repository=self.universal_notification_repository,
            user_repository=self.user_repository, session_registry=self.session_registry)
        self.universal_image_service = UniversalImageService(api_key=self.settings.gemini_api_key, model=self.settings.gemini_voice_model,
            pending_repository=self.universal_image_pending_repository, live_capture_service=self.universal_live_capture_service,
            openai_api_key=self.settings.openai_api_key, openai_model=self.settings.openai_vision_model,
            min_confidence=self.settings.image_ai_min_confidence)
        self.rag_service = RagService(self.rag_knowledge_repository); self.product_ai_desk_service = ProductAIDeskService(self.product_catalog_repository)
        self.buyer_intelligence_service = BuyerIntelligenceService(); self.decision_opportunity_service = DecisionOpportunityService()
        self.seller_ai_escalation_service = SellerAIEscalationService(self.seller_ai_escalation_repository, self.product_ai_desk_service,
            self.whatsapp_service, self._resolve_universal_contact)
        self.product_buyer_runtime_service = ProductBuyerRuntimeService(notification_repository=self.universal_notification_repository,
        ...
        self.catering_menu_ai_service = CateringMenuAIService(
            api_key=self.settings.gemini_api_key,
            catalog_repository=self.catering_catalog_repository,
            pending_repository=self.catering_menu_pending_repository,
        )
        ...
        self.conversation_service = UniversalAwareConversationService(response_commands=self.universal_response_command_service,
            ...
            catering_runtime=self.catering_rfq_runtime_service, catering_menu_ai=self.catering_menu_ai_service)
        ...
        self.voice_assistant_service = SarvamTTSVoiceAssistantService(sarvam_api_key=self.settings.sarvam_api_key,
            ...
            api_key=self.settings.gemini_api_key, model=self.settings.gemini_voice_model, max_audio_bytes=self.settings.gemini_voice_max_bytes,
            tts_model=self.settings.gemini_tts_model, tts_voice=self.settings.gemini_tts_voice, voice_reply_max_chars=self.settings.voice_reply_max_chars,
            transcription_attempts=2, generate_content_attempts=2, audio_codec_service=self.audio_codec_service)
        self.job_matching_service = JobMatchingService(user_repository=self.user_repository, whatsapp_service=self.whatsapp_service)
        self.job_lifecycle_service = JobLifecycleService(repository=self.job_lifecycle_repository, whatsapp_service=self.whatsapp_service)
        self.easy_job_command_service = EasyJobCommandService(repository=self.job_lifecycle_repository, lifecycle_service=self.job_lifecycle_service)
    ...
    def _resolve_universal_contact(self, user_id: str):
        ...
        whatsapp_mobile = str(user.get("whatsapp_mobile") or user_id); entered_mobile = str(user.get("entered_mobile") or whatsapp_mobile)
        return {**user, "mobile": whatsapp_mobile, "phone": entered_mobile, "whatsapp_mobile": whatsapp_mobile, "name": user.get("name") or "PODX User"}
    ...
    def _universal_profiles(self):
        ...
        for mobile,user in users.items():
            capabilities=self.user_repository.list_capabilities(mobile); role=str(user.get("role") or "").upper()
            if not role and capabilities:role="BOTH" if len(capabilities)>1 else str(capabilities[0]).upper()
            profiles.append({"user_id":mobile,"role":role,"category":user.get("job_category"),"skill":user.get("job_category"),"latitude":user.get("latitude"),"longitude":user.get("longitude")})
